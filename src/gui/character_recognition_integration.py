...
class CharacterRecognitionIntegration:
    ...
    def __init__(self, model_path: Optional[str] = None, buffer_size: int = 256):
        ...
        self.writing_buffer = {
            ...
            "force_raw": deque(maxlen=buffer_size),
            "battery_mv": deque(maxlen=buffer_size),
            "force_mv": deque(maxlen=buffer_size),
        }
    ...
    def load_model(self, model_path: str):
        ...
        try:
            ...
            self.preprocessor = SensorDataPreprocessor(max_timesteps=512, sampling_rate=208)

            # Load model with proper initialization
            self.model = CharacterRecognitionModel(timesteps=512, n_features=13, num_classes=26, model_path=model_path)
    ...
    def add_sensor_data(self, sensor_dict: Dict):
        ...
        with self.lock:
            ...
            if buffer_len <= 5:
                ...
                force_raw = sensor_dict.get("force_raw", 0)
                print(
                    f"  \ud83d\udce5 Sample {buffer_len} | t={timestamp} | accel_x={accel_x:.0f} | force_raw={force_raw:.0f}"
                )
    ...
    def end_writing(self) -> Tuple[Optional[str], float]:
        ...
        with self.lock:
            ...
            try:
                ...
                # Convert buffer to numpy array (15 channels: 3+3+3+3+3 IMUs + force raw + battery + force_mv)
                sensor_data = np.zeros((buffer_len, 15), dtype=np.float32)
                ...
                sensor_data[:, 12] = list(self.writing_buffer["force_raw"])  # Raw force (analog sensor)
                sensor_data[:, 13] = list(self.writing_buffer["battery_mv"])  # Battery voltage in mV
                sensor_data[:, 14] = list(self.writing_buffer["force_mv"])  # Force voltage in mV
                ...
                force_raw_data = sensor_data[:, 12]  # force_raw channel
                battery_data = sensor_data[:, 13]  # battery_mv channel
                force_mv_data = sensor_data[:, 14]  # force_mv channel
                print(f"\n   🔧 Force Sensor (Raw):")
                print(f"      Range: [{force_raw_data.min():.0f}, {force_raw_data.max():.0f}]")
                print(f"      Mean: {force_raw_data.mean():.0f}, Std: {force_raw_data.std():.2f}")
                print(f"      First 5 values: {force_raw_data[:5]}")
                print(f"\n   🔋 Battery Voltage (mV):")
                print(f"      Range: [{battery_data.min():.0f}, {battery_data.max():.0f}]")
                print(f"      Mean: {battery_data.mean():.0f}, Std: {battery_data.std():.2f}")
                print(f"      First 5 values: {battery_data[:5]}")
                print(f"\n   ⚡ Force Voltage (mV):")
                print(f"      Range: [{force_mv_data.min():.0f}, {force_mv_data.max():.0f}]")
                print(f"      Mean: {force_mv_data.mean():.0f}, Std: {force_mv_data.std():.2f}")
                print(f"      First 5 values: {force_mv_data[:5]}")
                ...
                # Now using 15 channels (12 IMU + 3 power/force measurements)
                sensor_data_normalized = self.preprocessor.normalize_data(sensor_data_padded.reshape(1, 512, 15))
                ...
                print(
                    f"   Normalized stats: mean={sensor_data_normalized.mean():.4f}, std={sensor_data_normalized.std():.4f}"
                )
                ...
                class_idx, confidence = self.model.predict_single(sensor_data_normalized[0], return_confidence=True)
