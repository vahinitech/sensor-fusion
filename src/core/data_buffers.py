...
class SensorBuffers:
    ...
    def _init_buffers(self):
        ...
        self.force_raw = deque(maxlen=self.buffer_size)
        self.battery_mv = deque(maxlen=self.buffer_size)
        self.force_mv = deque(maxlen=self.buffer_size)
    ...
    def calculate_magnitude(self, x_vals, y_vals, z_vals):
        ...
        if not x_vals or not y_vals or not z_vals:
            return []
        return [np.sqrt(x**2 + y**2 + z**2) for x, y, z in zip(x_vals, y_vals, z_vals)]
