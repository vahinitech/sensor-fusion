...
class ExtendedKalmanFilter:
    ...
    def __init__(self, 
                 process_variance: np.ndarray = None,
                 measurement_variance: np.ndarray = None,
                 dt: float = 0.0096):  # 104Hz sampling
        """
...
class SensorFusionManager:
    ...
    def _init_filters(self):
        ...
        if self.filter_type == 'kf':
            ...
            self.filters = {
                'accel': [KalmanFilter() for _ in range(3)],
                'gyro': [KalmanFilter() for _ in range(3)]
            }
        elif self.filter_type == 'ekf':
            self.filters = {
                'accel': ExtendedKalmanFilter(),
                'gyro': ExtendedKalmanFilter()
            }
        elif self.filter_type == 'cf':
            self.filters = {
                'accel': ComplementaryFilter(),
                'gyro': ComplementaryFilter()
            }
    ...
    def filter_accel(self, accel: Dict[str, float]) -> Dict[str, float]:
        ...
        if self.filter_type == 'kf':
            ...
            filtered = self.filters['accel'].update(
    ...
    def filter_gyro(self, gyro: Dict[str, float]) -> Dict[str, float]:
        ...
        if self.filter_type == 'kf':
            ...
            filtered = self.filters['gyro'].update(
