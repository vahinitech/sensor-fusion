...
class ModelTrainer:
    ...
    def prepare_data(self, X_raw, y_labels, test_size=0.2, validate=True):
        ...
        # Normalize
        X_normalized = self.preprocessor.normalize_data(X_processed, X_test=None, fit=True)
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, epochs=50, batch_size=32, validation_split=0.2):
        ...
        self.history = self.model.train(
            X_train, y_train, X_val=X_val, y_val=y_val, epochs=epochs, batch_size=batch_size, callbacks=callbacks
        )
    ...
    def evaluate(self, X_test, y_test):
        ...
        report = classification_report(y_test_chars, y_pred_chars, target_names=target_names)

        conf_matrix = confusion_matrix(y_test_chars, y_pred_chars)

        self.val_metrics = {"accuracy": accuracy, "report": report, "confusion_matrix": conf_matrix.tolist()}
